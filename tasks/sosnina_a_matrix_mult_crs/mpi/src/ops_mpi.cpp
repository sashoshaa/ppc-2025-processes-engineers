#include "sosnina_a_matrix_mult_crs/mpi/include/ops_mpi.hpp"
...
bool SosninaAMatrixMultCRSMPI::RunImpl() {
  ...
  int n_rows_A = 0, n_cols_A = 0, n_cols_B = 0;
...
bool SosninaAMatrixMultCRSMPI::RunSequential() {
  ...
  // Алгоритм умножения матриц в формате CRS (аналогичный seq версии)
...
bool SosninaAMatrixMultCRSMPI::PrepareAndValidateSizes(int &n_rows_A, int &n_cols_A, int &n_cols_B) {
  ...
  // Устанавливаем размеры на всех процессах
  n_rows_A_ = n_rows_A;
  n_cols_A_ = n_cols_A;
  n_cols_B_ = n_cols_B;

  if (n_rows_A <= 0 || n_cols_A <= 0 || n_cols_B <= 0) {
    return false;
  }
...
void SosninaAMatrixMultCRSMPI::BroadcastMatrixB() {
  // Определяем размеры данных для матрицы B
  int b_sizes[3] = {0, 0, 0};
  ...
  // Выделяем память под данные матрицы B
  if (rank_ != 0) {
  ...
  // Передаем данные матрицы B всем процессам
...
void SosninaAMatrixMultCRSMPI::DistributeMatrixAData() {
  // Определяем строки матрицы A для текущего процесса (циклическое распределение для горизонтальной схемы)
  local_rows_.clear();
  for (int i = 0; i < n_rows_A_; ++i) {
  ...
  if (rank_ == 0) {
    // Корневой процесс собирает и отправляет данные остальным процессам
    ...
    // Корневой процесс обрабатывает свои строки
    local_values_A_.clear();
    local_col_indices_A_.clear();
...
void SosninaAMatrixMultCRSMPI::ComputeLocalMultiplication() {
  // Вычисляем локальное умножение для назначенных строк
  int local_row_count = static_cast<int>(local_rows_.size());

  // Векторы для накопления локальных результатов
  ...
  for (int local_idx = 0; local_idx < local_row_count; ++local_idx) {
    ...
    for (int k_idx = row_start; k_idx < row_end; ++k_idx) {
      // Проверка границ
      if (k_idx < 0 || k_idx >= static_cast<int>(local_values_A_.size()) ||
          k_idx >= static_cast<int>(local_col_indices_A_.size())) {
        continue;
      }
      ...
      if (b_row_start < 0 || b_row_end > static_cast<int>(values_B_.size()) || b_row_start > b_row_end) {
        continue;
      }

      for (int j_idx = b_row_start; j_idx < b_row_end; ++j_idx) {
        // Проверка границ для индексов B
        if (j_idx < 0 || j_idx >= static_cast<int>(values_B_.size()) ||
            j_idx >= static_cast<int>(col_indices_B_.size())) {
          continue;
        }
    ...
    for (int j = 0; j < n_cols_B_; ++j) {
      if (std::abs(temp_row[j]) > 1e-12) {  // Проверка на ненулевое значение
        local_row_values[local_idx].push_back(temp_row[j]);
        local_row_cols[local_idx].push_back(j);
      }
    }

    // Сортируем по столбцам
    if (!local_row_cols[local_idx].empty()) {
      std::vector<std::pair<int, double>> pairs;
      for (size_t idx = 0; idx < local_row_cols[local_idx].size(); ++idx) {
        pairs.emplace_back(local_row_cols[local_idx][idx], local_row_values[local_idx][idx]);
      }
      ...
      // Обновляем отсортированные данные
      for (size_t idx = 0; idx < pairs.size(); ++idx) {
        local_row_cols[local_idx][idx] = pairs[idx].first;
        local_row_values[local_idx][idx] = pairs[idx].second;
      }
    }

    // Обновляем указатели на строки
    local_row_ptr_C_[local_idx + 1] = local_row_ptr_C_[local_idx] + local_row_cols[local_idx].size();
  }
...
void SosninaAMatrixMultCRSMPI::GatherResults() {
  if (rank_ == 0) {
    ...
    for (int src = 1; src < world_size_; ++src) {
      // Определяем строки для процесса src (циклическое распределение)
      ...
      if (received_row_count > 0) {
        // Получаем номера строк от процесса src (для правильного сопоставления)
        ...
        // Получаем локальные row_ptr_C от процесса src
        ...
        // Определяем общее количество ненулевых элементов
        ...
        // Распределяем данные по соответствующим строкам
    ...
    // Собираем данные в правильном порядке (по строкам 0, 1, 2, ...)
    values_C_.clear();
    col_indices_C_.clear();
    row_ptr_C_.resize(n_rows_A_ + 1, 0);
    row_ptr_C_[0] = 0;
    ...
    for (int i = 0; i < n_rows_A_; ++i) {
      ...
      // Добавляем значения и индексы для строки i
      values_C_.insert(values_C_.end(), row_values[i].begin(), row_values[i].end());
      col_indices_C_.insert(col_indices_C_.end(), row_cols[i].begin(), row_cols[i].end());

      // Обновляем указатель строки
  ...
  // Корневой процесс формирует выходные данные
  if (rank_ == 0) {
    OutType result = std::make_tuple(values_C_, col_indices_C_, row_ptr_C_);
    GetOutput() = result;
  } else {
...
void SosninaAMatrixMultCRSMPI::ConvertToDenseRow(const std::vector<double> &values, const std::vector<int> &col_indices,
                                                 const std::vector<int> &row_ptr, int row_index, int n_cols,
                                                 std::vector<double> &dense_row) {
  dense_row.assign(n_cols, 0.0);
  int start = row_ptr[row_index];
  int end = row_ptr[row_index + 1];
...
void SosninaAMatrixMultCRSMPI::ConvertDenseToCRS(const std::vector<double> &dense_row, std::vector<double> &values,
                                                 std::vector<int> &col_indices, int &row_start, int /* row_end */,
                                                 double epsilon) {
  row_start = static_cast<int>(values.size());

  for (size_t j = 0; j < dense_row.size(); ++j) {
    if (std::abs(dense_row[j]) > epsilon) {
      values.push_back(dense_row[j]);
