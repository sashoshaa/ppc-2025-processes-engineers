...
- Рассылает метаданные и матрицу B всем процессам через `MPI_Bcast`
- Вычисляет распределение строк матрицы A между процессами
- Отправляет строки матрицы A worker-процессам через `MPI_Send`
- Выполняет локальное умножение своей части матрицы A на B
- Собирает частичные результаты от worker-процессов через `MPI_Recv`
- Рассылает финальную результирующую матрицу всем процессам через `MPI_Bcast`
- Сохраняет результат в `GetOutput()`

**Процессы 1..N-1 (Worker-процессы):**
- Получают метаданные и матрицу B от процесса 0 через `MPI_Bcast`
- Получают свои строки матрицы A от процесса 0 через `MPI_Recv`
- Выполняют локальное умножение назначенных строк матрицы A на B
- Отправляют результаты процессу 0 через `MPI_Send`
- Получают финальный результат от процесса 0 через `MPI_Bcast`
- Сохраняют результат в `GetOutput()`
